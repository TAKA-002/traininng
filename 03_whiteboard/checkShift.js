...
//2021.01.01
//業務管理シートのシート名が「yyｙｙｍｍｄｄ」から「mmdd」に変更されていたため、修正
...
function getMemberCount() {
    ...
    var MemberCount = ss.getSheetByName(SheetName).getRange("A3").getValue();
    MemberCount = MemberCount - 6; //業務管理シートの木下さんまでの行数をひく
    return MemberCount;
}
...
function makeLastDaySheetName() {
    ...
    day = ("0" + day).slice(-2);
    var lastDaySheetName = month + day;
    return lastDaySheetName;//mmdd
}
...
function makeTodaySheetName() {
    ...
    month = ("0" + month).slice(-2);
    day = ("0" + day).slice(-2);
    var todaySheetName = month + day;
    return todaySheetName;//mmdd
}
...
function makeTomorrowSheetName() {
    ...
    month = ("0" + month).slice(-2);
    day = ("0" + day).slice(-2);
    var tomorrowSheetName = month + day;//ｍｍｄｄ
