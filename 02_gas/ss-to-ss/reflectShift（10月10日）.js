//変更箇所
const MEMBER_COUNT = 37;

//取得セル範囲
var SHIFT_START_ROW = 7;
var SHIFT_START_COLUMN = 2;
//COLUMN_INVALID_LINE = 51;
...
function chenge() {
  ...
  //アクティブなsheetの名前を取得して、「平日マスタ」or「休日マスタ」or「祝日マスタ」ならこのスクリプト何もせずは終了（確認OK）
  if (sheet.getName() == WEEKDAY_MASTER_SHEET_LABRL || sheet.getName() == HOLIDAY_MASTER_SHEET_LABRL || sheet.getName() == PUBLIC_HOLIDAY_MASTER_SHEET_LABRL) {
    return
  }
  ...
  //マスタ以外なら、セルの取得からsetTimeまでをループさせる
  //問題点：休でとまる（10月8日）

  var AreaRanges = sheet.getRange(SHIFT_START_ROW, SHIFT_START_COLUMN, COLUMN_INVALID_LINE);
  ...
  for (var i = 1; i <= AreaCount; i++) {
    ...
    //取得したセルが２カラム目（B列）以外なら終了。51行目よりしたの行なら終了
    ...
    // LABEL_POSITIONの値「平日」「休日」「祝日」のどれかを取得
...
function setTime(row, value, sheet, masterSheet) {
  // 最後の行を取得
  var lastRow = masterSheet.getLastRow();//「平日マスタ」「休日マスタ」「祝日マスタ」のシートの最終行を取得
  // セルをひとつ指定（「番号１：番号２」という書き方は、番号１〜番号２までということ）
  var list = masterSheet.getRange(COLUMN_NAME + 1 + ":" + COLUMN_TOKUSHU2_END + lastRow).getValues();
  //COLUMN_NAME（A列）＋１＝「A1」。COLUMN_TOKUSHU2_END（AM列）＋最終行の数字（平日マスタなら18）＝AM18
  //A1:AM18の値を二次元配列で取得する。それをlistに格納。
  var masterRow = -1;
  for (var i0 = 0; i0 < list.length; i0++) {//変数i0に０を初期値。listの数だけ繰り返す。おそらく1回
    if (list[i0][INDEX_NAME] == value) {//なにをするか。listの一次元インデックス０のINDEX_NAME（0）が「cell.getValue()」なら、0を代入。
      masterRow = i0;
      break;
    }
  }
