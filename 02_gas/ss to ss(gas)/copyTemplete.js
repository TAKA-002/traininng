...
function getMakingMonth() {
  //仮想シフト表のIDを取得する
  const ss = SpreadsheetApp.openById('1YWp9IUB1LGed4LZGYd2DNKvQx4RqSTozmWYs-hSiAWI');//★★★①シフト表のIDを入力★★★
...
function copyTemplete(){
  ...
  //テンプレートとなる業務管理シートを取得
  var templeteFile = DriveApp.getFileById('1lsxixwYUSTfWOStrcM2mmocPjgyFvNyTbj2B395T6WU');//★★★⑦テンプレートの業務管理シートファイルIDを入力★★★
  
  //出力先のフォルダーを取得
  var OutputFolder = DriveApp.getFolderById('1ekDBmDsLyo8gqxxRenS3neNhz6Sg0W91');//★★★⑧出力先フォルダーIDを入力★★★
  
  //出力する時のファイル名を指示する
  var OutputFileName = templeteFile.getName().replace('仮想業務管理シート', '業務管理シート_' + makingYear + makingMonth);//★★★⑨replace('テンプレートのファイル名', '新ファイル名'）を入力★★★
